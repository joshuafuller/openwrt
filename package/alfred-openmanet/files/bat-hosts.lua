...
-- then return a table containing valid bat-hosts lines
  ...
  for mac, iname in pairs(ifaces) do
    -- Don't add invalid MAC addresses
    -- Don't add morse0 MAC addresses (12:00:00:00:00:00)
    if mac:match("^%x%x:%x%x:%x%x:%x%x:%x%x:%x%x$") and not mac:match("00:00:00:00:00:00") and not mac:match("12:00:00:00:00:00") then
